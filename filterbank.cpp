...
//		else if (strtmp == "mcnt")
//		{
//			fread (&Mcnt, sizeof (Mcnt), 1, fpInput);
//			intTotalHeaderBytes += sizeof (Mcnt);
//		}
...
//	put_string(f_fil, "mcnt");
//	fwrite (&Mcnt, sizeof (Mcnt), 1, f_fil);
...
//void WriteHeader(FILE * fname)
{
	...
	double Fch1 = 800;
	double Foff = -100.0/N_CHANS_SPEC;
	int Nchans  = N_CHANS_SPEC;
	float Tsamp = float(T_RSL)/1000/1000;//(s )
	int Nbits   = N_BITS_DATA_POINT;
	int Nifs    = N_POLS_PKT;
	int start   = time((time_t*)NULL);
	double current_MJD;
	double Year	= File_time.Year;
	double Month	= File_time.Month;
	double Day	= File_time.Day;
	double Hour	= File_time.Hour;  
	double Min	= File_time.Min;
	double Sec	= File_time.Sec;
	double Usec	= File_time.Usec;
        double jd;
	int beam_id;
	char beamid[50];
	sprintf(beamid, "%c%c\0",*(fname+22),*(fname+23));
//	sprintf(beamid, "%c%c\0",*(fname+23),*(fname+24));
	beam_id = atoi(beamid)+1;
//	printf("String ID:%s,Beam ID:%d, File:%s\n",beamid,beam_id,fname);
	jd = UTCtoJD(Year, Month, Day);
	current_MJD= jd+(double)((Hour-12)/24.0)
                               +(double)(Min/1440.0)
                               +(double)(Sec/86400.0)
                               +(double)(Usec/86400.0/1000000.0)
                               -(double)2400000.5;

	FilterBankData fil;
	strcpy(fil.Source_name, "TianLai_Test");
